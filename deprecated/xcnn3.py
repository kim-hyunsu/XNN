...
class XLift(nn.Module):
    ...
    def __call__(self, x):
        ...
        for x in x_list:
            ...
            K = self.kernel_size
            S = self.strides
            out_size = [int((H[i]-K)/S+1) for i in range(D)] # TODO consider padding
            ...
            for j in range(D-1):
                ...
                n = jax.image.resize(n, shape=(B, *out_size, C), method="nearest")
                n = Norm(n)
                n = Act(n)
    ...
    def __call__(self, x_list):
        ...
        for i, x in enumerate(x_list):
            ...
            for j, y in enumerate(x_list):
                ...
                m = jax.image.resize(m, shape=(B, *out_size, C), method="nearest")
                m = Norm(m)
                m = Act(m)
